...
public final class LatticeMixinPlugin implements IMixinConfigPlugin {
    ...
    public void onLoad(final String mixinPackage) {
        // Sodium rewrites LevelRenderer; our redirect in fix.render.LevelRendererMixin collides with
        // their mixin. If Sodium/Iris is present, skip that mixin. On Forge this looked for the
        // Embeddium/Oculus/Rubidium forks of the same code.
        sodiumPresent =
            isModLoaded("sodium")
                || isModLoaded("iris")
                || isClassPresent("me.jellysquid.mods.sodium.client.render.SodiumWorldRenderer");
        disableFixMixins =
    ...
    public boolean shouldApplyMixin(final String targetClassName, final String mixinClassName) {
        ...
        if (sodiumPresent && isSodiumSensitiveRenderMixin(mixinClassName)) {
            logSkip(mixinClassName, "Sodium/Iris compatibility");
            return false;
        }
        return true;
    }
    ...
    private static boolean isSodiumSensitiveRenderMixin(final String mixinClassName) {
